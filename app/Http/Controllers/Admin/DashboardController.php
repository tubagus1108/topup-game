...
class DashboardController extends Controller
{
    ...
    protected function calculate($model, $date, $type, $status = null)
    {
        // Key for the cache
        $key = "calculate_{$model}_{$date->day}_{$date->month}_{$date->year}_{$type}_{$status}";
        ...
        $query = $model::whereDay('created_at', $date->day)
    ...
    {
        return $this->calculate(Pembelian::class, $date, $type, $status);
    }

    protected function calculatePembayaran($date, $type, $status)
    {
        return $this->calculate(Pembayaran::class, $date, $type, $status);
    }
    
    protected function getMorrisData($grafik)
    {
        // Tambahkan pengecekan untuk memastikan $grafik tidak kosong
        if ($grafik->isEmpty()) {
            return [];
        }

        // Key for the cache
        $start_date = $grafik->first()->created_at;
        $end_date = $grafik->last()->created_at;

        $key = "getMorrisData_{$start_date->timestamp}_{$end_date->timestamp}";
